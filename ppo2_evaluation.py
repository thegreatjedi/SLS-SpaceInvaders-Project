...
def train(game, num_timesteps, num_envs, dir_name, model_name,
    ...
    if prev_model_name is not None and os.path.exists(prev_model_path):
        model = PPO2.load(prev_model_path, env=env)
        model.tensorboard_log = log_dir
    else:
        model = PPO2(policy="MlpLnLstmPolicy", env=env, gamma=0.1, n_steps=128,
                     learning_rate=0.001, verbose=1, tensorboard_log=log_dir)
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        while retry:
            try:
                train(game=game_name, num_timesteps=1000000, num_envs=4,
                      dir_name="ppo2", model_name=f"{curr_model_iter}",
    ...
    for i in range(5):
        ...
        while retry:
            try:
                evaluate(game=game_name, num_eps=100, num_envs=4,
                         dir_name="ppo2", model_name=f"{curr_model_iter}")
                retry = False
            except EOFError:
                pass
